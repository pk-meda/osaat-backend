...
from .serializers import PatientComplaintSerializer, SecondscreeningSerializer, ComprehensiveSerializer, UserSerializer, FirstScreeningSerializer, schoolSerializer, InitialEyeTestSerializer, RetestEyeTestSerializer,  DispensingSerializer, RefractionExaminationSerializer, DiagnosisSerializer, ParticipantSerializer, SpectacleHistorySerializer, SurgeryTreatmentHistorySerializer, OtherMedicalIssueResponseSerializer, FamilyHistorySerializer, CurrentMedicalSerializer, VisualacuitySerializer, RefractionSpectacleSerializer
...
from .models import  PatientComplaint, SecondScreening, Country, Province, Comprehensive,Firstscreening, Dispensing, school, RefractionExamination, Diagnosis, Participant, SpectacleHistory, SurgeryTreatmentHistory, FamilyHistory,  RefractionSpectacle, CurrentMedicalTreatment, VisualAcuityMeasurement, OtherMedicalIssueResponse
...
class UserLoginView(APIView):
    def post(self, request):
        ...
        if not email or not password:
            return Response({'body': [], 'message': 'Email and password are required', 'error': True}, status=status.HTTP_400_BAD_REQUEST)

        try:
            user = User.objects.get(email=email)
        except User.DoesNotExist:
            return Response({'body': [], 'message': 'Invalid email or password', 'error': True}, status=status.HTTP_400_BAD_REQUEST)
...
class UserLogoutView(APIView):
    ...
    def post(self, request):
        try:
            ...
            return Response({'body': [], 'message': 'Logout successful', 'error': False}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'body': [], 'message': 'Something went wrong', 'error': True}, status=status.HTTP_400_BAD_REQUEST)
...
class SendOTPView(APIView):
    def post(self, request):
        ...
        if not email:
            return Response({"body": [], "message": "Email is required", "error": True}, status=status.HTTP_400_BAD_REQUEST)

        try:
            user = User.objects.get(email=email)
        except User.DoesNotExist:
            return Response({"body": [], "message": "User not found", "error": True}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"body": [], "message": "OTP sent to your email", "error": False}, status=status.HTTP_200_OK)
...
class VerifyOTPView(APIView):
    def post(self, request):
        ...
        if not email or not otp:
            return Response({"body": [], "message": "Email and OTP are required", "error": True}, status=status.HTTP_400_BAD_REQUEST)

        stored_otp = cache.get(f"otp_{email}")

        if stored_otp is None or stored_otp != otp:
            return Response({"body": [], "message": "Invalid or expired OTP", "error": True}, status=status.HTTP_400_BAD_REQUEST)

        cache.delete(f"otp_{email}")

        return Response({"body": [], "message": "OTP verified successfully", "error": False}, status=status.HTTP_200_OK)
...
class ResetPasswordView(APIView):
    def post(self, request):
        ...
        if not email or not new_password:
            return Response({"body": [], "message": "Email and new password are required", "error": True}, status=status.HTTP_400_BAD_REQUEST)

        try:
            user = User.objects.get(email=email)
            user.set_password(new_password)
            user.save()
            return Response({"body": [], "message": "Password reset successfully", "error": False}, status=status.HTTP_200_OK)
        except User.DoesNotExist:
            return Response({"body": [], "message": "User not found", "error": True}, status=status.HTTP_400_BAD_REQUEST)
...
class DispensingAPIView(APIView):
    ...
    def get(self, request):
        ...
        """
        reference_number = request.GET.get("reference_number")  # Get reference_number from query params
        ...
        latest_dispensing_record = Dispensing.objects.filter(reference_number=reference_number).order_by('-id').first()  # Get latest record
        ...
        serializer = DispensingSerializer(latest_dispensing_record)  # Serialize the latest record
...
class RefractionExaminationAPIView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            participant = Participant.objects.get(reference_number=reference_number)
            participant.refraction_and_examination = True  # Set refraction examination to True
            participant.save()
        except Participant.DoesNotExist:
        ...
        return Response({
            "body": request.data,
            "message": "Refraction Examination created successfully" if created else "Refraction Examination updated successfully",
            "error": False
        }, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
    ...
    def get(self, request, *args, **kwargs):
        ...
        refraction_exam = RefractionExamination.objects.filter(reference_number=reference_number).order_by('-id').first()

        if not refraction_exam:
            return Response({
                "message": "Refraction Examination record not found",
                "error": True
            }, status=status.HTTP_404_NOT_FOUND)
...
class DiagnosisView(APIView):
    def post(self, request):
        ...
        return Response({
            "message": "Diagnosis saved successfully" if created else "Diagnosis updated successfully",
            "error": False
        }, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
...
class ParticipantView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            first_screening = Firstscreening.objects.get(reference_number=reference_number)
        ...
        try:
            second_screening = SecondScreening.objects.get(reference_number=reference_number)
...
class SpectacleHistoryAPIView(APIView):
    def post(self, request):
        ...
        try:
            participant = Participant.objects.get(reference_number=reference_number)
            participant.spectacle_wearing_history = True  # Assuming this field exists in Participant model
            participant.save()
        except Participant.DoesNotExist:
            pass  # If Participant does not exist, do nothing

        return Response({
            "message": "Spectacle history saved successfully" if created else "Spectacle history updated successfully",
            "error": False
        }, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
    def get(self, request):
        """
        ...
        try:
            spectacle_history = SpectacleHistory.objects.get(reference_number=reference_number)
            ...
            return Response({
                "message": "Spectacle history record not found",
                "error": True
            }, status=status.HTTP_404_NOT_FOUND)
...
class SurgeryTreatmentHistoryAPIView(APIView):
    def post(self, request):
        ...
        return Response({
            "message": "SurgeryTreatmentHistory saved successfully" if created else "SurgeryTreatmentHistory updated successfully",
    ...
    def get(self, request):
        ...
        surgery_history = get_object_or_404(SurgeryTreatmentHistory, reference_number=reference_number)
...
class OtherMedicalIssueResponseView(APIView):
    def get(self, request):
        ...
        medical_issue = OtherMedicalIssueResponse.objects.filter(reference_number__iexact=reference_number.strip()).first()
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                participant = Participant.objects.get(reference_number=reference_number)
                participant.other_medical_issues = obj.other_medical_issues  # Update field
                participant.save()
            except Participant.DoesNotExist:
                pass  # If Participant does not exist, do nothing
...
class FamilyHistoryAPIView(APIView):
    def post(self, request):
        ...
        try:
            participant = Participant.objects.get(reference_number=reference_number)
            participant.family_history = True  # Assuming you have this field in Participant model
            participant.save()
        except Participant.DoesNotExist:
            pass
        ...
        return Response({
            "body": response_data,
            "message": "Family History saved successfully" if created else "Family History updated successfully",
            "error": False
        }, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
    def get(self, request):
        ...
        # Fetch the latest record for the given reference_number
        family_history = FamilyHistory.objects.filter(reference_number=reference_number).order_by('-id').first()
        ...
        response_data = {
            ...
            "SerachTerm": family_history.search_term if family_history.search_term else ""
        }
...
class CurrentMedicalAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            participant = Participant.objects.get(reference_number=reference_number)
            participant.current_medical_treatment = True  # Assuming this field exists in Participant model
            participant.save()
        except Participant.DoesNotExist:
            pass  # If Participant does not exist, do nothing

        return Response({
            "message": "CurrentMedicalTreatment saved successfully" if created else "CurrentMedicalTreatment updated successfully",
            "error": False
        }, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
    ...
    def get(self, request):
        ...
        # Fetch the record, if it exists
        current_medical = get_object_or_404(CurrentMedicalTreatment, reference_number=reference_number)
...
class VisualAcuityMeasurementAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            participant = Participant.objects.get(reference_number=reference_number)
            participant.measure_visual_acuity = True  # Assuming this field exists in Participant model
            participant.save()
        except Participant.DoesNotExist:
            pass  # If Participant does not exist, do nothing

        return Response({
            "message": "Measure Visual acuity saved successfully" if created else "Measure Visual acuity updated successfully",
            "error": False
        }, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
    ...
    def get(self, request):
        ...
        measurement = get_object_or_404(VisualAcuityMeasurement, reference_number=reference_number)  # Fetch the record
        serializer = self.serializer_class(measurement)  # Serialize the record

        return Response({"body": serializer.data, "error": False}, status=status.HTTP_200_OK)
...
class RefractionSpectacleAPIView(APIView):
    def post(self, request):
        ...
        try:
            participant = Participant.objects.get(reference_number=reference_number)
            participant.refraction_spectacle_presentation = True  # Assuming this field exists in Participant model
            participant.save()
        except Participant.DoesNotExist:
            pass  # If Participant does not exist, do nothing

        return Response({
            "message": "RefractionSpectacle saved successfully" if created else "RefractionSpectacle updated successfully",
            "error": False
        }, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
    ...
    def get(self, request):
        ...
        # Fetch the record, if it exists
        refraction_spectacle = get_object_or_404(RefractionSpectacle, reference_number=reference_number)
...
class FailedStudents(APIView):
    def get(self, request):
        ...
        failed_students = Firstscreening.objects.filter(school=school, first_screening_passed=False)
...
# class ReferenceNumberCheckAPIView(APIView):
    def post(self, request):
        ...
        if not file:
            return Response({"message": "No file uploaded", "error": True}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            existing_reference_numbers = set()  # Store reference numbers already in the database
            pattern = re.compile(r"^[A-Za-z0-9]+$")  # Match alphanumeric reference numbers (e.g., DGF288401)

            # Loop through all rows and cells to find all valid reference numbers
            for row in sheet.iter_rows(values_only=True):
                for cell in row:
                    if isinstance(cell, str) and pattern.match(cell):  # Check for valid reference number
                        ref_number = cell.strip()
                        # Check if reference number already exists in the database
                        if Firstscreening.objects.filter(reference_number=ref_number).exists():
...
class AliExpressProductSearchAPIView(APIView):
    def post(self, request):
        ...
        if not keyword:
            return Response({"error": "Missing 'keyword' in request body"}, status=status.HTTP_400_BAD_REQUEST)
